...
// Nova rota para obter o status das integrações
router.get('/status', ensureAuthenticated, integrationController.getIntegrationStatus);
...
const uploadMediaToGithub = async (file, type, github) => {
 ...
 const path = require("path")
    ...
    try {
      ...
      if (!bucketExists) {
        ...
        try {
          ...
          console.error('Erro ao configurar política do bucket:', policyError);
        }
      }
      ...
      // Gera a URL pública do MinIO
      const mediaUrl = `https://dev.hocketzap.com/integrations/${bucketName}/${objectName}`;  // Substitua 'seu-ip' pelo IP do seu servidor
  ...
  router.get('/chat-media/:type/:filename', async (req, res) => {
    const bucketName = 'chat-media';
